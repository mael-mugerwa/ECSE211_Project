...
public class Project {
  ...
  public static final SampleProvider rotatingLightSensorProvider =
      rotatingLightSensor.getMode("RGB");
  ...
  public static void main(String[] args) throws OdometerExceptions {
    ...
    // Odometer related objects
    final CanScanner canScanner = CanScanner.getCanScanner(leftMotor, rightMotor, rotatingArmMotor,
        gateMotor, rotatingLightSensorProvider, usSensorProvider);
    ...
    final FinalNavigation navigation = FinalNavigation.getNavigation(leftMotor, rightMotor,
        usSensorProvider, canScanner, leftLightSensor, rightLightSensor);

    // Localization related objects

    final UltrasonicLocalizer usLocalization =
        new UltrasonicLocalizer(leftMotor, rightMotor, usSensor);
    // final US us = new US( odometer, leftMotor, rightMotor, usSensorProvider);
    // final LightLocalizer lightLocalization = new LightLocalizer(leftMotor, rightMotor,
    // rightLightSensor);
    final FullLocalization fullLocl = new FullLocalization(leftMotor, rightMotor, usSensorProvider,
        leftLightSensorProvider, rightLightSensorProvider);
    ...
    (new Thread() {
      public void run() {
        ...
        // Sound.beep();
        // }
        navigation.RegularTravelTo(0, 2);
